...
import { removeText } from '../services/geminiService';
...
const RemoveText: React.FC = () => {
    ...
    const handleRemove = async () => {
        ...
        try {
            ...
            const result = await removeText({ data: base64Data, mimeType: baseImage.file.type });
    ...
    const downloadResult = () => {
        ...
        link.download = `${originalName}-no-text.png`;
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="text-center">
                <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">Công cụ Xóa Văn bản</h2>
                <p className="mt-4 text-lg leading-8 text-gray-400">
                    Xóa mọi văn bản khỏi hình ảnh của bạn một cách chính xác trong khi vẫn giữ nguyên nền.
                </p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                    <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                        ...
                        <p className="text-sm text-gray-400 mb-3">Chọn một ảnh chứa văn bản cần xóa.</p>
                    ...
                    >
                        {loading ? <Spinner /> : <EraserIcon />}
                        <span>{loading ? 'Đang xóa văn bản...' : 'Xóa Văn bản'}</span>
                    </button>
                </div>
                    ...
                    <div className="aspect-square w-full rounded-lg border-2 border-dashed border-gray-700 bg-gray-800 flex items-center justify-center relative group overflow-hidden">
                        ...
                        {!loading && !resultImage && (
                             <div className="text-center text-gray-500 p-4">
                                <EraserIcon className="mx-auto h-12 w-12" />
                                <p className="mt-2 text-sm">Ảnh đã xóa văn bản sẽ xuất hiện ở đây.</p>
                            </div>
                         )}
                    </div>
                </div>
